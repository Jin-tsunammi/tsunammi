		...
		if task.SourceTokenMint.Equals(solana.WrappedSol) {
			...
			if amountIn.Cmp(spendableLamports) > 0 {
				if minTransAmount.Cmp(spendableLamports) > 0 {
					errs[index] = fmt.Errorf(
						"insufficient SOL for min tx amount wallet %s balance %d spendable %d needed %d: %w",
						task.PrivateKey.PublicKey().String(),
						account.Lamports,
						spendableLamports,
						minTransAmount,
...
func (c *Client) buildSwapTransaction(
	...
	if isSolToToken { // BUY token
		expectedWSOLATA, _, err := solutil.FindAssociatedTokenAddressWithProgram(
			task.PrivateKey.PublicKey(),
			solana.WrappedSol,
...
func calculateAMMPrice(pool *PoolStateWithReserve, params raydium.SwapParams) *big.Rat {
	if pool.PoolState.BaseMint.Equals(params.InputTokenMint) && pool.PoolState.QuoteMint.Equals(params.OutputTokenMint) {
	...
	return poolmath.ConstantProductCalculatePrice(pool.ReserveB, pool.ReserveA, uint64(pool.BaseMintDecimals), uint64(pool.QuoteMintDecimals))
}
