...
func (c *Client) Swap(
	...
	if task.UsingJito {
		...
		if err != nil {
			// TODO: is needed? returns err anyways
...
func (c *Client) buildSwapTransaction(
	...
	params.InputTokenProgramID = solana.TokenProgramID
	params.OutputTokenProgramID = solana.TokenProgramID
...
func calculateAMMPrice(pool *AMMInfoWithReservers, params raydium.SwapParams) *big.Rat {
	if pool.PoolState.CoinMint.Equals(params.InputTokenMint) {
		// pulldown: ReserveA=coin=token, ReserveB=pc=SOL -> SOL/token
	...
	// pullup: ReserveA=pc=SOL, ReserveB=coin=token -> SOL/token
	return poolmath.ConstantProductCalculatePrice(pool.ReserveB, pool.ReserveA, pool.PoolState.CoinDecimals, pool.PoolState.PcDecimals)
}
