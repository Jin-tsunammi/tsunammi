...
export const sliceNumberAfterDot = (val) => {
    if (isNaN(+val)) return 0;

    if (String(val).includes('.')) {
        const splitVal = String(val).split('.');
        const slicedNum = splitVal[1].slice(0, 4);

        if (Number(slicedNum) > 0) {
            return Number(`${splitVal[0]}.${slicedNum}`);
        } else {
            return splitVal[0];
        }
    } else {
...
export function toDynamicFix(value, precision = 4) {
    ...
    const fix = firstSignificantIndex + set;
...
export const formatSmallNumbers = (value) => {
    const num = Number(value);

    if (!Number.isFinite(num)) return '0';
    const str = String(num);
    if (str.includes('e')) {
        const zeroNumber = str.split('-')[1];

        return String(Number(str).toFixed(+zeroNumber));
    } else {
        return num;
    }
}

export const normilizeCampaignStatus = (status) => {
    if (!status) return '';
    const formattedStatus = status.toLowerCase().replaceAll('_', ' ');
